...
Notation "{ x 'in' A | P }E" := (separation_cons A (fun x => P))
...
Lemma separation_cons_separates : forall (A : Ensemble) (P : Ensemble -> Prop) x,
	(x inE A) -> P x -> (x inE {x in A | P x}E).
Proof.
...
Notation "{ A ';' B }E" := (pairing_cons A B) (at level 69, only parsing).
Notation "{ A  ;  B }" := (pairing_cons A B) (at level 69, only printing).
Notation "{ A }E" := (singleton A) (at level 69, format "{ A }E").
(* For all Ensemble A,
...
Qed.
Lemma pairing_trans : forall A B C D : Ensemble, A =E C -> B =E D -> {A ; B}E =E {C ; D}E.
Proof.
...
Qed.
Lemma pairing_sym : forall A B : Ensemble, {A ; B}E =E {B ; A}E.
Proof.
	intros A B; split; intros [|];
	  [exists rightopt | exists leftopt | exists rightopt | exists leftopt];
	  by exact (Eeq_refl _).
Qed.
Lemma pairing_inj : forall A B C : Ensemble, {A ; C}E =E {B ; C}E -> A =E B.
Proof.
	intros A B C ACeqBC.
...
Lemma singleton_has_elem : forall A : Ensemble, A inE {A}E.
Proof.
	by intros [A f]; exists leftopt; exact (Eeq_refl _).
Qed.
Lemma singleton_is_singleton : forall A B : Ensemble, B inE {A}E -> B =E A.
Proof.
	by intros A B BinsA;
	  specialize (pairing_is_lr BinsA) as [H | H];
	  exact H.
Qed.
Lemma singleton_incl_is_in : forall A B : Ensemble, {A}E incl B -> A inE B.
Proof.
	intros A B sAinclB.
	by exact (sAinclB _ (singleton_has_elem A)).
Qed.
Lemma in_is_singleton_incl : forall A B : Ensemble, A inE B -> {A}E incl B.
Proof.
	intros A B AinB x xinA.
	by exact (Eeq_in_trans_r (singleton_is_singleton _ _ xinA) AinB).
Qed.
Lemma singleton_refl : forall A B : Ensemble, A =E B -> {A}E =E {B}E.
Proof.
	...
	by exact (singleton_has_elem _).
Qed.
Lemma singleton_inj : forall A B : Ensemble, {A}E =E {B}E -> A =E B.
Proof.
	intros A B sAeqsB.
	by exact (singleton_is_singleton _ _ (singleton_incl_is_in _ _ (Eeq_incl_l sAeqsB))).
Qed.
Lemma singleton_union_singleton : forall A B : Ensemble, ({A}E) \union ({B}E) =E {A ; B}E.
Proof.
...
Definition Succ (A : Ensemble) := A \union ({A}E).
...
Definition two_tuple A B := {{A}E ; {A ; B}E}E.
...
Proof.
	intros A B ABeqBA.
	specialize (pairing_trans_r _ _ ({B}E) (pairing_sym A B)) as B_ABeqB_BA.
...
Definition partial_application_set (A B : Ensemble) : Ensemble :=
	{ f in cartesian A B |
	 forall x y x' y', x =E x' -> E(x, y) inE f -> E(x', y') inE f -> y =E y' }E.
Definition total_application_set (A B : Ensemble) : Ensemble :=
	{ f in partial_application_set A B |
	 forall y, y inE B -> exists x, x inE A /\ E(x, y) inE f }E.
